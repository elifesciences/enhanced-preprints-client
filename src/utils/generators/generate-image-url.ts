...
export const generateImageUrl = (contentImageId: string): string => (config.imageServer ? `${config.imageServer?.replace(/\/$/, '')}/${encodeURIComponent(contentImageId)}/full/max/0/default.jpg` : contentImageId);

export const generateImageUrlSized = (contentImageId: string, size: number): string => {
  if (!config.imageServer) return contentImageId;

  return `${config.imageServer?.replace(/\/$/, '')}/${encodeURIComponent(contentImageId)}/full/${size},/0/default.jpg`;
};

export const generateImageInfo = async (contentImageId: string): Promise<{ width: number, height: number }> => {
  const infoUrl = `${config.iiifServer?.replace(/\/$/, '')}/${encodeURIComponent(contentImageId)}/info.json`;
