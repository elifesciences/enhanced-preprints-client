...
export const generateTimeline = (version: EnhancedArticleWithVersions): TimelineEvent[] => {
  ...
  const versions = Object.values(version.versions).sort((a, b) => new Date(b.published).getTime() - new Date(a.published).getTime());
  const timeline: TimelineEvent[] = versions.map<TimelineEvent>((current) => (current.id === version.article.id ?
    {
