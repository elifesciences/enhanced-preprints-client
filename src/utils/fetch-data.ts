...
export const fetchVersionsNoContent = async (page: number, perPage: number, order: 'asc' | 'desc', useDate: 'default' | 'published', startDate: string, endDate: string) => {
  const url = `${config.apiServer}/api/preprints-no-content?page=${page}&per-page=${perPage}&order=${order}${useDate === 'published' ? '&use-date=firstPublished' : ''}${startDate ? `&start-date=${startDate}` : ''}${endDate ? `&end-date=${endDate}` : ''}`;
  return fetch(url)
    .then(async (response) => {
      if (!response.ok) {
