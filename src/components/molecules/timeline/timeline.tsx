      ...
      <dl className={`review-timeline${expanded !== false ? ' review-timeline--expanded' : ''}`} id="review-timeline" aria-label="Version history">
        {
          sortedEvents.map((event, index) => {
            ...
            return (
              <Fragment key={index}>
                <dt className={`review-timeline__event review-timeline__event--title${typeEventClass}${activeEventClass}${statusEventClass}${evaluationSummaryClass}`}{...(hidden ? { style: { display: 'none' } } : {})}>
                ...
                <dd className={`review-timeline__event review-timeline__event--detail${typeEventClass}${activeEventClass}${statusEventClass}${evaluationSummaryClass}`}{...(hidden ? { style: { display: 'none' } } : {})}>
