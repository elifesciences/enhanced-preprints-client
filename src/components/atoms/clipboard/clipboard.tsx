  ...
  return (
    <div className="clipboard-container">
      <div className="form-item">
          ...
          {(showButton ? <button className={`button button--default${copied ? ' copied' : ''}`} onClick={() => {setCopied(true); navigator.clipboard.writeText('https://doi.org/10.7554/eLife.09560');}}>{copied ? 'copied' : 'copy to clipboard'}</button> : <></>)}
      </div>
    </div>
  );
};
