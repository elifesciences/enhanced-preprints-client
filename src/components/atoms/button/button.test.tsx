...
describe('Button', () => {
  ...
  it('renders with the provided text and iconName', () => {
    ...
    expect(screen.getByText('button text')).toBeInTheDocument();
    expect(screen.getByText('button text')).toHaveClass('button--icon');
    expect(screen.getByText('button text')).toHaveClass('button--icon-dark_mode');
    expect(screen.getByText('button text')).toHaveAttribute('href');
    expect(screen.getByText('button text').getAttribute('href')).toStrictEqual('bbc.co.uk');
  });

  it('renders with the provided text, iconName and variant', () => {
    render(<Button text="button text" iconName="dark_mode" variant="dark_mode" url='bbc.co.uk' />);

    expect(screen.getByText('button text')).toBeInTheDocument();
    expect(screen.getByText('button text')).toHaveClass('button--icon');
    expect(screen.getByText('button text')).toHaveClass('button--icon-dark_mode');
    expect(screen.getByText('button text')).toHaveClass('button--dark_mode');
    expect(screen.getByText('button text')).toHaveAttribute('href');
    expect(screen.getByText('button text').getAttribute('href')).toStrictEqual('bbc.co.uk');
  });
});
